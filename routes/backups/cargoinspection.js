...
const cargoInspectionController = require('../controllers/cargoInspectionController.js');
...
router.post('/get', (req, res, next) => {
	async.waterfall([
		...
		function gettingData (index, callback) {
			cargoInspectionController.find(req.APP, req, (err, result) => {
...
router.post('/total', (req, res, next) => {
	async.waterfall([
		...
		function gettingData (index, callback) {
			cargoInspectionController.count(req.APP, req, (err, result) => {
				if (err) return callback(err);
...
router.post('/delete', (req, res, next) => {
	async.waterfall([
		...
		function checkingParameters (index, callback) {
			if ( (!req.body.id) && (!req.body.cargoinspection_id) ) return callback({
					code: 'MISSING_KEY',
					data: req.body,
					info: {
						missingParameter: 'id'
		...
		function gettingData (index, callback) {
			cargoInspectionController.delete(req.APP, req, (err, result) => {
				if (err) return callback(err);
...
router.post('/update', (req, res, next) => {
	async.waterfall([
		...
		function updateData (index, callback) {
			cargoInspectionController.update(req.APP, req, (err, result) => {
				if (err) return callback(err);
				callback(null, result);
			});
		}
...
router.post('/insert', (req, res, next) => {
	async.waterfall([		
		...
		function insertData (index, callback) {
			cargoInspectionController.insert(req.APP, req, (err, result) => {				
				if (err) return callback(err);
				callback(null, result);
			});
		}		
